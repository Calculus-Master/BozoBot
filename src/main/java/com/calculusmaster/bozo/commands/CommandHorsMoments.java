...
public class CommandHorsMoments extends Command
{
    ...
    {
        Document data = Mongo.UserMomentsDB.find(Filters.eq("type", "hors")).first();

        if(data == null) return this.error("Hors Moments database JSON not found!");

        List<String> attachmentPool = data.getList("attachments", String.class);
        List<String> queuedPool = data.getList("queued", String.class);

        if(attachmentPool.isEmpty()) return this.error("No Hors Moments are available yet!" + (queuedPool.isEmpty() ? " None are in queue!" : " There are " + queuedPool.size() + " moments waiting to be added!"), true);
